...
CHANNEL_ACCESS_TOKEN = os.environ.get("CHANLEL_ACCESS_TOKEN", "")
...
def push_message(group_id, messages):
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {CHANNEL_ACCESS_TOKEN}"}
    resp = requests.post(PUSH_URL, headers=headers, json={"to": group_id, "messages": messages}, timeout=10)
...
def forward_event(event):
    ...
    if not TARGET_GROUP_IDS:
        print("[DEBUG] TARGET_GROUP_IDS not set")$        return
    msg = event.get("message", {})
    msg_type = msg.get("type")
    forward_messages = []
    if msg_type == "text":
        forward_messages = [{"type": "text", "text": "[Forward]\n" + msg.get("text", "")}]
    elif msg_type == "sticker":
        forward_messages = [{"type": "sticker", "packageId": msg.get("packageId"), "stickerId": msg.get("stickerId")}]
    else:
        forward_messages = [{"type": "text", "text": f"[Forward] Received {msg_type}"}]
...
def health():
    return "LINE Forward Bot Running", 200
